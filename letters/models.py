...
# def add_mytask(instance, periodicity):
#     # Логика для добавления задачи
#     print(f"Задача добавлена для рассылки: {instance} с периодичностью: {periodicity}")
...
class MailingAttempt(models.Model):
    ...
    attempt_time = models.DateTimeField(default=timezone.now, verbose_name='Дата и время последней попытки')
...
class Mailing(models.Model):
    ...
    first_send_time = models.DateTimeField(default=timezone.now, verbose_name='Дата и время первой отправки')
    ...
    customers = models.ManyToManyField(Customer, related_name='mailings_cistomers', verbose_name='Клиенты')

    # Связь с моделью Message
    messages = models.ForeignKey(Message, on_delete=models.DO_NOTHING, related_name='mailings_messages', verbose_name='Сообщение')

    # Связь с моделью MailingAttempt
    attemtps = models.ManyToManyField(MailingAttempt, related_name='mailings_attempts', verbose_name='Попытки', blank=True)
    def __str__(self):
...
def mailing_customers_changed(sender, instance, action, **kwargs):
    if action in ['post_add', 'post_remove', 'post_clear']:
        ...
        add_mytask(instance.id, instance.periodicity, customer_emails, message_emails)
